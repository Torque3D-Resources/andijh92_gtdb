...
# -------------------------------------------------------------------------------
        ...
        self.list_watchValue = ListLignForEditor(watchValue, -1,
                _('_name-file'), _('_name-attribute'), _('_value'))
        watchValue_sizer.Add(self.list_watchValue, 1, wx.EXPAND
                             | wx.ALL)
        watchValue.SetSizer(watchValue_sizer)

        callStack_sizer = wx.BoxSizer(wx.VERTICAL)
        self.list_callStack = ListLignForEditor(callStack, -1,
                _('_name-stack'), _('_func-stack'), '')
        ...
        self.list_breakPoint = ListLignForEditor(breakPoint, -1,
                _('_name-lib'), _('_condition-lib'), _('_hit-lib'))
        breakPoint_sizer.Add(self.list_breakPoint, 1, wx.EXPAND
                             | wx.ALL)
        breakPoint.SetSizer(breakPoint_sizer)

        logDisplay_sizer = wx.BoxSizer(wx.VERTICAL)
        self.list_logDisplay = LogDisplay(logDisplay, -1)
        logDisplay_sizer.Add(self.list_logDisplay, 1, wx.EXPAND
                             | wx.ALL)
        logDisplay.SetSizer(logDisplay_sizer)

        logCompile_sizer = wx.BoxSizer(wx.VERTICAL)
        self.list_logCompile = ListLignForEditor(logCompile, -1,
                _('_name-error'), _('_error-lib'), _('_compile-mess'))
        logCompile_sizer.Add(self.list_logCompile, 1, wx.EXPAND
                             | wx.ALL)
    ...
    def Populate(
        ...
        if self.param:
            if noInit:
                li = []
                x = \
                    self.getparents().mainFrame.menu.getParameter(self.param)
                if x:
                    li = x

                li.append(col1)
                self.getparents().mainFrame.menu.setParameter(self.param,
                        li)
...
class LogDisplay(wx.TextCtrl):
    ...
    def __init__(self, parent, ID):
        wx.TextCtrl.__init__(self, parent, ID, size=(400, 90),
    ...
    def append(self, data):
        ...
        for l in lis:
            ...
            if l.startswith('COUT '):
                ...
                if l.find('nable to load') > -1:
                    if lineprevious:
                        ...
                        self.getparents().mainFrame.nboo.list_logCompile.Populate('Missing object'
                                , 'Unable to load', l)
                        self.getparents().mainFrame.SetStatusText(_('_error-compile'
                                ))
                else:
                ...
                if l.find('No mission specified') > -1:
                    self.getparents().mainFrame.nboo.list_logCompile.Populate('parameters wrong'
                            , 'Missing mission', l)
                    self.getparents().mainFrame.SetStatusText(_('_error-compile'
                            ))
                    continue
            ...
            if l.startswith('PASS Connected'):
                liste = \
                    self.getparents().mainFrame.menu.getParameter('break'
                        )
            ...
            if l.startswith('EVALOUT') and l.find('DUMP') < 0:
                lis = l.split(' ')
                pop = lis[1].split('|')
                self.getparents().mainFrame.nboo.list_watchValue.Populate(pop[0]
                        + ':' + pop[1], pop[2], lis[2])
                continue
    ...
    def displayBreak(self, l):
        ...
        val = l.split(' ')
        fic = \
            os.path.join(self.getparents().mainFrame.menu.getEnvDir(),
                         val[0])
        filL = val[0]
        ...
        if j > 0:
            ...
            while i >= 0:
                self.getparents().mainFrame.nboo.list_callStack.Populate(val[i
                        - 2] + ':' + val[i - 1], val[i], '')
                i = i - 3
        ...
        if self.firstTime:
            ...
            MsgDlg(self.getparents().mainFrame,
                   _('_message-debug-ready-lib'),
                   _('_message-debug-ready'), wx.OK
                   | wx.ICON_INFORMATION)
